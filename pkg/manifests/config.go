...
import (
	...
	cmometrics "github.com/openshift/cluster-monitoring-operator/pkg/metrics"
)
...
func NewConfig(content io.Reader, tp bool) (*Config, error) {
	...
	if tp {
		...
		for _, profile := range SupportedCollectionProfiles {
			...
			cmometrics.ConfiguredCollectionProfile.WithLabelValues(string(profile)).Set(v)
		}
		if didMisconfigureCollectionProfile {
